<template>
    <main>
        <section class="container-fluid p-0">
            <div class="jumbotron jumbotron-fluid d-flex align-items-center text-center text-white bg-dark p-5"
                style="background-image: linear-gradient(rgba(0, 0, 0, 1), rgba(0, 0, 0, 0.5)), url('https://source.unsplash.com/random/?movies'); background-size: cover; background-position: center; height: 50vh">
                <div class="container">
                    <h1 class="display-4 fw-bold">{{ siteName }}</h1>
                    <p class="lead">Un catalogue de films à déguster avec de déclicieux popcorns !</p>
                    <router-link v-if="!isLoggedIn" :to="{ name: 'register' }" class="btn btn-primary btn-lg p-3 mt-3 fw-bold">Je
                        m'inscris</router-link>
                </div>
            </div>
        </section>
        <div class="container">
...
export default {
    ...
    computed: {
        ...mapGetters(['siteName', 'isLoggedIn'])
    },
    created() {
        this.fetchMovies();
    }
